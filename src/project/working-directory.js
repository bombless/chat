...
class WorkdirManager {
  constructor(options = {}) {
    this.file = options.file || path.resolve(options.appRoot || __dirname, '.workdirs.json');
    this.policy = {
  ...
  pending() { return [...this._pending.values()].map(({ id, path: requestPath, createdAt }) => ({ id, path: requestPath, createdAt })); }
  ...
  add(input) {
    ...
    if (!this._workdirs.some(item => item.path === workdir)) { this._workdirs.push({ path: workdir }); this._save(); }
    return workdir;
  }
  switch(input) { const workdir = this.validate(input); this.add(workdir); this._current = workdir; return workdir; }
  createApprovalRequest(input) { const workdir = this.validate(input); const id = crypto.randomUUID(); this._pending.set(id, { id, path: workdir, createdAt: Date.now() }); return { id, path: workdir, status: 'pending' }; }
  ...
  _load() {
    try { if (!fs.existsSync(this.file)) return []; const items = JSON.parse(fs.readFileSync(this.file, 'utf8')); return Array.isArray(items) ? items.filter(item => item && typeof item.path === 'string') : []; }
    catch (error) { console.warn('⚠️ 读取工作目录列表失败，将重新创建:', error.message); return []; }
  }
  _save() { try { fs.writeFileSync(this.file, JSON.stringify(this._workdirs, null, 2)); } catch (error) { console.warn('⚠️ 保存工作目录列表失败:', error.message); } }
}
